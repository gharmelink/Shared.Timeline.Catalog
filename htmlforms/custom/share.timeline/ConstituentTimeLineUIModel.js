				...
				d3.csv(container.fixUrl("/util/DataList.ashx?DatabaseName=BBInfinity&dataListID="+communicationDatalistID+"&ContextRecordID=" + CONTEXTID + "&format=csv&DATEFILTER=10"))
					...
					.get(function(error, communications) {					
						$.getScript( "../browser/htmlforms/custom/shared_resources/vis/vis.js", function( data, textStatus, jqxhr ) {
							...
							interActionTypes=uniqueBy(ints, function(x){return x.interactiontype;});
							recognitionTypes=uniqueBy(recognitions, function(x){return x.application;});
							communicationTypes=uniqueBy(comms, function(x){return x.communicationtype;});
							...
							//menu hooks
							document.getElementById(util.getMappedElId(modelInstanceId, 'zoomIn')).onclick    = function () { timeline.zoomIn( 0.3); };
							document.getElementById(util.getMappedElId(modelInstanceId, 'zoomOut')).onclick   = function () { timeline.zoomOut( 0.3); };
							document.getElementById(util.getMappedElId(modelInstanceId, 'moveLeft')).onclick  = function () { move( 0.3,timeline); };
							document.getElementById(util.getMappedElId(modelInstanceId, 'moveRight')).onclick = function () { move(-0.3,timeline); };
						
							//add nodes and click actions
							//add interactions
							$.each( interActionTypes, function( index, value ) {
								tree1.tree('addNode', {li: {'class': 'leaf'},input:{'type':'checkbox','id':util.getMappedElId(modelInstanceId, "interaction"+value.replace(/\s/g,'')+"Check")},span:{'html':value}}, $('#'+util.getMappedElId(modelInstanceId, 'interactionList')));
							...
							$.each( recognitionTypes, function( index, value ) {
								tree1.tree('addNode', {li: {'class': 'leaf'},input:{'type':'checkbox','id':util.getMappedElId(modelInstanceId, "recognition"+value.replace(/\s/g,'')+"Check")},span:{'html':value}}, $('#'+util.getMappedElId(modelInstanceId, 'recognitionList')));
							...
							$.each( communicationTypes, function( index, value ) {								
								tree1.tree('addNode', {li: {'class': 'leaf'},input:{'type':'checkbox','id':util.getMappedElId(modelInstanceId, "communication"+value.replace(/\s/g,'')+"Check")},span:{'html':value}}, $('#'+util.getMappedElId(modelInstanceId, 'communicationList')));
